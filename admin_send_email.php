...
function post_notification_admin_sub() { ?>
    ...
    <p> Der Emailversand ist momentan deaktiviert. In der n&auml;chsten version wird er wahrscheinlich wieder vorhanden
        sein.</p>
	<?php
}
...
function post_notification_for_future_use() {
	...
	while ( $row = mysql_fetch_assoc( $result_email_list ) ) {
		...
		$site_name = str_replace( array(
			...
			utf8_encode( 'ü' ),
			utf8_encode( 'ß' )
		), array( 'Ä', 'ä', 'Ö', 'ö', 'Ü', 'ü', 'ß' ), $site_name );
		$header    = "From: \"" . $site_name . "\" <$from_email>\n";

		$subject = stripslashes( $subject );
		$subject = str_replace( array( "â€œ", "â€?", "â€™", "â€“", "â€”", "â€¦", "&nbsp;" ), array(
			'"',
			'"',
			'´',
			'–',
		...
		$message = str_replace( array( "â€œ", "â€?", "â€™", "â€“", "â€”", "â€¦", "&nbsp;" ), array(
			'"',
			'"',
			'´',
			'–',
