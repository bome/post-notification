...
function pn_send_mail( $to, $subject, $message, $headers = array(), $attachments = array() ) {
    ...
    // Default: WordPress — ensure UTF-8 and correct content type
    // Detect content type from headers (falls back to HTML if message looks like HTML)
    $is_html = false;
    foreach ( $headers as $k => $v ) {
        if ( is_string( $k ) && strcasecmp( $k, 'Content-type' ) === 0 && stripos( (string) $v, 'text/html' ) !== false ) {
            $is_html = true; break;
        }
        if ( is_string( $v ) && stripos( $v, 'Content-Type:' ) === 0 && stripos( $v, 'text/html' ) !== false ) {
            $is_html = true; break;
        }
    }
    if ( ! $is_html && preg_match( '/<\w+[^>]*>/', (string) $message ) ) {
        $is_html = true;
    }
